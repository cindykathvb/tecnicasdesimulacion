...
def inventario_EOQ():
    ...
    if request.method == 'POST' and form.validate():
        ...
        indice = ['I','Orden','Reparar','Total','Diferencia']
        ...
        for period in periodo:
            dfQ['Orden'][period] = D * Co / dfQ['I'][period]
            dfQ['Reparar'][period] = dfQ['I'][period] * Ch / 2
            dfQ['Total'][period] = dfQ['Orden'][period] + dfQ['Reparar'][period]
            dfQ['Diferencia'][period] = dfQ['Total'][period] - MOQ
        pd.set_option('mode.chained_assignment', None)
        df = dfQ


        dfG = dfQ.loc[:,'Orden':'Total']
        dfG
        dfG.plot()
...
def inventario_general():
    ...
    if request.method == 'POST' and form.validate():
        ...
        def make_data(product, policy, periods):
            ...
            # Abbreviations
            # Inicial: Neto inicialtial inventory position
            # Neto inicial: Neto inicialtial net inventory
            # D: Demand
            # Final: Final inventory position
            # NETO: Final net inventory
            # LS: Lost sales
            # AVG: Average inventory
            # ORD: order quantity
            # LT: lead time
            header = ['Inicial','Neto inicial','DEMANDA', 'Final',
            'NETO', 'VENTAS', 'Promedio', 'Cantidad', 'TIEMPO']
            ...
            for period in period_lst:
                if period == 0:
                    df['Inicial'][period] = product.initial_inventory
                    df['Neto inicial'][period] = product.initial_inventory
                    df['Final'][period] = product.initial_inventory
                    df['NETO'][period] = product.initial_inventory
                if period >= 1:
                    df['Inicial'][period] = df['Final'][period - 1] + order_l[period - 1].quantity
                    df['Neto inicial'][period] = df['NETO'][period - 1] + pending_order(order_l, period)
                    ...
                    if demand > 0:
                        ...
                        df['DEMANDA'][period] = 0
                    # We can't have negative Inicial
                    if df['Inicial'][period] - df['DEMANDA'][period] < 0:
                        df['Final'][period] = 0
                    else:
                        df['Final'][period] = df['Inicial'][period] - df['DEMANDA'][period]
                    order_l[period].quantity, order_l[period].lead_time = placeorder(product, df['Final'][period], policy,
                    period)
                    df['NETO'][period] = df['Neto inicial'][period] - df['DEMANDA'][period]
                    if df['NETO'][period] < 0:
                        df['VENTAS'][period] = abs(df['NETO'][period])
                        df['NETO'][period] = 0
                    else:
                        df['VENTAS'][period] = 0
                        df['Promedio'][period] = (df['Neto inicial'][period] + df['NETO'][period]) / 2.0
                        df['Cantidad'][period] = order_l[period].quantity
    ...
    if request.method == 'POST' and form.validate():
        ...
        for i in Clientes:
            if i == 0:
                ...
                dfLE['TIESA'][i] = dfLE['TIESP'][i] + dfLE['TISE'][i]
        nuevas_columnas = pd.core.indexes.base.Index(["Llegada Cliente","Servicio","Tiempo llegada","Tiempo servicio",
        "Tiempo exacto","Inicio servicio","Fin servicio",
        "Tiempo espera","Tiempo sistema"])
    ...
    if request.method == 'POST' and form.validate():
        ...
        for i in Clientes:
            if i == 0:
                ...
                dfLE['TIESA'][i] = dfLE['TIESP'][i] + dfLE['TISE'][i]
        nuevas_columnas = pd.core.indexes.base.Index(["Llegada Cliente","Servicio","Tiempo llegada","Tiempo servicio",
        "Exacto llegada","Inicio servicio","Fin servicio",
        "Tiempo espera","Tiempo sistema"])
        dfLE.columns = nuevas_columnas
        df = dfLE
        df2 = dfLE.describe()
